...
import com.smoothstack.dao.AdminBorrowerDAO;
import com.smoothstack.main.UI;
import com.smoothstack.models.Borrower;


public class AdminBorrowerService implements LibraryService {
	...
	private int getUniqueCardNumber() {
		...
		while(flag) {
			...
			AdminBorrowerDAO.getInstance().selectAll();
	...
	public void add() {		
		...
		AdminBorrowerDAO.getInstance().addBorrower(borr);					
	}
	...
	public void update() {
		...
		while(flag && borr != null) {
			...
			AdminBorrowerDAO.getInstance().updateBorrower(borr, oldCardNumber);										
			flag = false;
		}		
	...
	public void delete() {
		...
		while(flag && borr != null) {
			...
			if("y".equalsIgnoreCase(choice)) {
				AdminBorrowerDAO.getInstance().deleteBorrower(borr);
				flag = false;
			}
			else if("n".equalsIgnoreCase(choice)){
	...
	private Borrower getBorrChoice() {
		...
		while(flag) {
			...
			Borrower borr = AdminBorrowerDAO.getInstance().getById(choice);
